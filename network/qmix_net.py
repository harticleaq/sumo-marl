...
class QMixNet2(nn.Module):
    ...
    def forward(self, q_values, states, other_actions, agent_id):  # states的shape为(episode_num, max_episode_len， state_shape)
        # 传入的q_values是三维的，shape为(episode_num, max_episode_len， n_agents)
        episode_num = q_values.size(0)
        q_values = q_values.view(-1, 1, self.args.n_agents)  # (episode_num * max_episode_len, 1, n_agents) = (1920,1,5)
        b_states = states.reshape(-1, self.args.state_shape)  
        states = states.reshape(-1, self.args.state_shape).repeat(1, self.args.n_agents).view(-1, self.args.n_agents, self.args.state_shape) # (episode_num * max_episode_len, state_shape)
        ...
        w1 = torch.abs(self.hyper_w1(states))  # (1920, 160)
        b1 = self.hyper_b1(b_states)  # (1920, 32)
        ...
        w2 = torch.abs(self.hyper_w2(b_states))  # (1920, 32)
        b2 = self.hyper_b2(b_states)  # (1920, 1)
