...
def td_lambda_target(batch, max_episode_len, q_targets, args):
    ...
    terminated = (1 - batch["terminated"].float())
    r = batch['r']
    # --------------------------------------------------n_step_return---------------------------------------------------
    '''
    ...
    '''
    targets = r + args.gamma * q_targets * (1 - terminated)
    return targets
    n_step_return = torch.zeros((episode_num, max_episode_len, 1, max_episode_len))
    ...
    '''
    lambda_return = torch.zeros((episode_num, max_episode_len, 1))
    for transition_idx in range(max_episode_len):
        returns = torch.zeros((episode_num, 1))
        for n in range(1, max_episode_len - transition_idx):
