...
from network.qtran_net import QtranQBase
...
class QLMIX:
    def __init__(self, args):
        ...
        if self.args.load_model:
            ...
            if os.path.exists(self.model_dir + 'rnn_net_params.pkl'):
                ...
                path_qlmix = self.model_dir + 'qlmix_params.pkl'
                map_location = 'cuda:0' if self.args.cuda else 'cpu'
                self.eval_rnn.load_state_dict(torch.load(path_rnn, map_location=map_location))
                self.eval_joint_q.load_state_dict(torch.load(path_joint_q, map_location=map_location))
                self.qlmix.load_state_dict(torch.load(path_qlmix,map_location=map_location))
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        mix_q,_,_ = self.qlmix(d_q, s, is_v=True)
        mix_q = mix_q.squeeze(-1)
    ...
    def save_model(self, train_step):
        ...
        torch.save(self.qlmix.state_dict(), self.model_dir + '/' + num + '_qlmix_params.pkl')
