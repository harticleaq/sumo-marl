...
from network.qtran_net import QtranQBase,QMixNet
from network.qtran_net import QtranV
...
class QtranBase:
    def __init__(self, args):
        ...
        self.model_dir = f"C:\haq_project\StarCraft-master\StarCraft-master\model\{args.alg}\\{args.map}\\"
        # 如果存在模型则加载模型
        if self.args.load_model:
            print(self.model_dir)
            if os.path.exists(self.model_dir + '63_rnn_net_params.pkl'):
                path_rnn = self.model_dir + '63_rnn_net_params.pkl'
                path_joint_q = self.model_dir + '63_joint_q_params.pkl'
                path_v = self.model_dir + '63_v_params.pkl'
                path_qmix = self.model_dir + '63_qmix_params.pkl'
                ...
                self.qmix.load_state_dict(torch.load(path_qmix,map_location=map_location))
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        u, r, avail_u, avail_u_next, terminated, s, s_next = batch['u'], batch['r'],  batch['avail_u'], \
                                                  batch['avail_u_next'], batch['terminated'], batch['s'], batch['s_next']
        ...
        opt_action_eval = individual_q_evals.argmax(dim=3, keepdim=True)
        ...
        # q_opt = individual_q_clone.max(dim=-1, keepdim=True)[0].squeeze(-1)
        # mix_q = self.qmix(q_individual, s).squeeze(-1)
        # mix_q_opt = self.qmix(q_opt, s).squeeze(-1)
        # # mix_q = q_individual.sum(dim=-1)
        # # mix_q_opt = q_opt.sum(dim=-1)
        #
        #
        # joint_q_hat_opt, _, _ = self.get_qtran(batch, hidden_evals, hidden_targets, opt_onehot_eval, hat=True)
        # opt_error = mix_q_opt - joint_q_hat_opt.detach() + v  # 计算l_opt时需要将joint_q_hat_opt固定
        # l_opt = ((opt_error * mask) ** 2).sum() / mask.sum()
        #
        # nopt_error = mix_q - joint_q_evals.detach() + v  # 计算l_nopt时需要将joint_q_evals固定
        # nopt_error = nopt_error.clamp(max=0)
        # l_nopt = ((nopt_error * mask) ** 2).sum() / mask.sum()
        # q_error = mix_q.squeeze(-1) - (mix_q_opt.squeeze(-1) + joint_q_evals - joint_q_hat_opt).detach()
        # q_error = q_error.clamp(max=0)
        # l_q = ((q_error * mask) ** 2).sum() / mask.sum()

      #   q_total_targets =  r.squeeze(-1) + self.args.gamma * individual_q_targets.max(dim=-1)[0].sum(dim=-1) * (
      #                    1 - terminated.squeeze(-1))
      # # 计算l_nopt时需要将joint_q_evals固定
      #   # nopt_error = nopt_error.clamp(max=0)
      #   nopt_error = q_individual.sum(dim=-1) + joint_q_evals.detach() - q_total_targets
      #   l_nopt = ((nopt_error * mask) ** 2).sum() / mask.sum()
        ...
        # q_opt = individual_q_evals.max(dim=-1, keepdim=True)[0].squeeze(-1)

        q_opt = individual_q_clone.max(dim=-1, keepdim=True)[0].squeeze(-1)
        # q_opt2 = individual_q_evals.max(dim=-1)[0]

        # 最优动作loss
        # mix_q_opt = self.qmix(q_opt, s).squeeze(-1)
        mix_q_opt = q_opt.sum(dim=-1)
        # opt_error = mix_q - joint_q_evals.detach()
        opt_error = mix_q_opt - joint_q_hat_opt.detach()
        # opt_error = v - q_opt.detach()
        l_opt = ((opt_error * mask) ** 2).sum() / mask.sum()

        # 非最优动作loss
        # q_opt_copy = copy.deepcopy(q_opt)
        d_q = q_individual - q_opt
        # print('q',q_individual,'q_opt',q_opt)
        # print('joint_q',joint_q_evals,'joint_q_opt',joint_q_hat_opt)
        # d_q = q_individual - q_opt_copy
        mix_q = self.qmix(d_q, s).squeeze(-1)
        # mix_q = d_q.sum(dim=-1)

        q_error = (mix_q) - (joint_q_evals - joint_q_hat_opt).detach()
        # print('q_error',q_error)
        # q_error = mix_q.squeeze(-1) - joint_q_evals.detach()
        # q_error = q_error.clamp(max=0)
    ...
    def get_qtran(self, batch, hidden_evals, hidden_targets, local_opt_actions, hat=False):
        ...
        if hat:
            ...
            v = v.view(episode_num, -1, self.n_agents)
    ...
    def save_model(self, train_step):
        ...
        torch.save(self.qmix.state_dict(), self.model_dir + '/' + num + '_qmix_params.pkl')
