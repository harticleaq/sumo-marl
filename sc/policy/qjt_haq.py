...
from network.qjt_haq import Qjt_net, QtranV
...
class Qjt_haq:
    def __init__(self, args):
        ...
        self.eval_joint_q = Qjt_net(args)  # counterfactual joint networks
        self.target_joint_q = Qjt_net(args)
        self.v = QtranV(args)
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        mask2 = 1 - batch["padded"].float().repeat(1, 1, self.n_agents*self.n_actions).reshape(mask.shape[0],
                                    mask.shape[1],self.n_agents,self.n_actions)
        if self.args.cuda:
            u = u.cuda()
            r = r.cuda()
        ...
        joint_q_opt_evals = joint_q_opt_evals - torch.min(joint_q_opt_evals,-1, keepdim=True)[0]  # (episode个数, max_episode_len， n_agents)

        # 因为QTRAN-alt要对每个agent都计算l_opt，所以要把q_sum_opt和v再增加一个agent维
        individual_q_evals[avail_u == 0.0] = 999999
        # joint_q_opt_evals[avail_u==0.0] = -20
        q_opt = torch.max(individual_q_clone,dim=-1,keepdim=True)[0]
        opt_error = individual_q_evals - joint_q_opt_evals.detach() + v  # 计算l_opt时需要将joint_q_opt_evals固定

        l_opt = ((opt_error * mask2) ** 2).sum() / mask2.sum()
    ...
    def get_qtran(self, batch, local_opt_actions, hidden_evals, hidden_targets=None, hat=False):
        ...
        if self.args.cuda:
            ...
            if not local_opt_actions is None:
        ...
        if hat:
            ...
            v = v.view(episode_num, max_episode_len, -1, self.n_actions)
    ...
    def save_model(self, train_step):
        ...
        torch.save(self.eval_joint_q.state_dict(), self.model_dir + '/' +  'joint_q_params.pkl')
        torch.save(self.v.state_dict(), self.model_dir + '/' + 'v_params.pkl')
