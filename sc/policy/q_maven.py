...
import os
from network.maven_net import HierarchicalPolicy, BootstrappedRNN, VarDistribution, QtranQBase
...
class MAVEN:
    def __init__(self, args):
        ...
        self.qtran_net = QtranQBase(args)
        ...
        if self.args.load_model:
            if os.path.exists(self.model_dir + '/rnn_net_params.pkl'):
                ...
                path_q = self.model_dir + '/q_net_params.pkl'
                ...
                self.qtran_net.load_state_dict(torch.load(path_q,map_location=map_location))
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        joint_q_evals = self.get_qtran(batch, hidden_evals, hidden_targets)

        # 版本1
        targets = r + self.args.gamma * q_total_target * (1 - terminated)
        td_error = (q_total_eval + joint_q_evals - targets.detach())
        masked_td_error = mask * td_error  # 抹掉填充的经验的td_error
        ql_loss1 = (masked_td_error ** 2).sum() / mask.sum()

        # -------------------------------------------------QL Loss------------------------------------------------------

        # loss = rl_loss + self.args.lambda_mi * mi_loss + self.args.lambda_ql * ql_loss
        loss = self.args.lambda_mi * mi_loss + self.args.lambda_ql * ql_loss1
    ...
    def get_qtran(self, batch, hidden_evals, hidden_targets):
        ...
        q_evals = self.qtran_net(states, hidden_evals, u_onehot)
    ...
    def save_model(self, train_step):
        ...
        torch.save(self.qtran_net.state_dict(), self.model_dir + '/' + num + '_q_net_params.pkl')
