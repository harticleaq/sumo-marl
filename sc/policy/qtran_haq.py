...
class QtranAlt:
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        d = d.min(dim=-1)[0].min(dim=-1)[0]
        l_nopt = ((d * mask.min(dim=-1)[0]) ** 2).sum() / mask.min(dim=-1)[0].sum()
        # ---------------------------------------------L_nopt-----------------------------------------------------------

        # 因为QTRAN-alt要对每个agent都计算l_opt，所以要把q_sum_opt和v再增加一个agent维

        # simulate_q = joint_q_evals - torch.max(joint_q_evals,dim=-1,keepdim=True)[0]
        # d_haq = individual_q_evals - simulate_q.detach()
        # l_haq = ((d_haq * mask2) ** 2).sum() / mask.sum()
        # individual_q_evals[avail_u == 0.0] = 999999
        # error_d = individual_q_evals-joint_q_evals.detach()/self.n_agents
        # d = torch.clamp(error_d,max=0)
        # l_haq = ((d * mask2) ** 2).sum() / mask2.sum()


        # print('l_td is {}, l_opt is {}, l_nopt is {}'.format(l_td, l_opt, l_nopt))
        loss = 1/2*(l_td + self.args.lambda_opt * l_opt + self.args.lambda_nopt * l_nopt)
