...
from network.coma_critic import ComaCritic, Critic
...
class MAPG:
    def __init__(self, args):
        ...
        self.eval_critic = ComaCritic(self.obs_shape + self.n_actions + self.n_agents, self.args)
        # self.target_critic = ComaCritic(critic_input_shape, self.args)
        self.joint_critic = Critic(critic_input_shape, self.args)
        self.target_joint_critic = Critic(critic_input_shape, self.args)
        if self.args.cuda:
            self.eval_rnn.cuda()
            self.eval_critic.cuda()
            self.joint_critic.cuda()
            self.target_joint_critic.cuda()
        ...
        if args.optimizer == "RMS":
            ...
            self.joint_critic_optimizer = torch.optim.RMSprop(self.joint_critic.parameters(), lr=args.lr_critic)
        self.args = args
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        joint_q = self._train_critic(batch, max_episode_len, train_step)  # 训练critic网络，并且得到每个agent的所有动作的Ｑ值
        joint_q = joint_q.repeat(1, 1, self.n_agents)
        
        q_values = self._get_q_values(batch, max_episode_len)
        action_prob, entropy = self._get_action_prob(batch, max_episode_len, epsilon)  # 每个agent的所有动作的概率
        ...
        # 计算advantage
        baseline = (q_values).mean(dim=3, keepdim=True).squeeze(3).detach()
        advantage = (q_taken - baseline).detach()
        loss = - ((advantage * log_pi_taken) * mask).sum() / mask.sum()

        target_q = (joint_q + entropy).detach()
        # target_q = (joint_q).detach()
        q_loss = self.quantile_regression(target_q, q_taken)
    ...
    def _get_critic_inputs(self, batch, transition_idx, max_episode_len):
        ...
        u_onehot = u_onehot.view((episode_num, 1, -1))
        u_onehot_next = u_onehot_next.view((episode_num, 1, -1))

        if transition_idx == 0:  # 如果是第一条经验，就让前一个动作为0向量
            u_onehot_last = torch.zeros_like(u_onehot)
        else:
            u_onehot_last = batch['u_onehot'][:, transition_idx - 1]
            u_onehot_last = u_onehot_last.view((episode_num, 1, -1))
        ...
        '''
        # inputs.unsqueeze(0).expand(episode_num, -1, -1)
        # inputs_next.unsqueeze(0).expand(episode_num, -1, -1)
    ...
    def _get_q_values(self, batch, max_episode_len):
        ...
        for transition_idx in range(max_episode_len):
            obs = batch['o'][:, transition_idx].view((episode_num, self.n_agents, -1))
            u_onehot = batch['u_onehot'][:, transition_idx].view((episode_num, self.n_agents, -1))
            agent_id = torch.eye(self.n_agents).unsqueeze(0).expand((episode_num, -1, -1)).view((episode_num, self.n_agents, -1))
            inputs = torch.cat([obs, agent_id, u_onehot], -1)
            if self.args.cuda:
                inputs = inputs.cuda()
    ...
    def _get_action_prob(self, batch, max_episode_len, epsilon):
        ...
        entropy = -torch.sum(logits*action_prob, -1)
        if self.args.cuda:
    ...
    def _train_q(self, batch,  max_episode_len, train_step):
        u = batch['u']
        ...
        for transition_idx in range(max_episode_len):
            ...
            # 神经网络输入的是(episode_num * n_agents, inputs)二维数据，得到的是(episode_num * n_agents， n_actions)二维数据
            q_eval = self.joint_critic(inputs)
            q_target = self.target_joint_critic(inputs_next)
    ...
    def _train_critic(self, batch, max_episode_len, train_step):
        ...
        q_evals, q_next_target = self._train_q(batch, max_episode_len, train_step)
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.joint_critic.parameters(), self.args.grad_norm_clip)
