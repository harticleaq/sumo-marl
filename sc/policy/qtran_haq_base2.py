...
from network.qtran_net import QtranQBase2 as QtranQBase
from network.qtran_net import QtranV, QMixNet2
...
class QtranBase:
    def __init__(self, args):
        ...
        if self.args.load_model:
            if os.path.exists(self.model_dir + '/10_rnn_net_params.pkl'):
                path_rnn = self.model_dir + '/10_rnn_net_params.pkl'
                path_joint_q = self.model_dir + '/10_joint_q_params.pkl'
                path_v = self.model_dir + '/10_v_params.pkl'
        ...
        print('Init alg QTRAN_haq-base2')
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        u, r, avail_u, avail_u_next, terminated, s, s_next = batch['u'], batch['r'], batch['avail_u'], \
                                                             batch['avail_u_next'], batch['terminated'], batch['s'], \
                                                             batch['s_next']
        ...
        # 版本1
        q_individual = torch.gather(individual_q_evals, dim=-1, index=u)
        q_sum = self.qmix_net(torch.cat((q_individual, joint_q_evals), dim=-2), s)
        q_sum_target = self.qmix_net(
            torch.cat((individual_q_targets.max(dim=-1, keepdim=True)[0], joint_q_targets), dim=-2), s_next)
        q_total_targets = r.squeeze(-1) + self.args.gamma * q_sum_target.squeeze(-1) * (1 - terminated.squeeze(-1))
        nopt_error = q_sum.squeeze(-1) - q_total_targets.detach()  # 计算l_nopt时需要将joint_q_evals固定
        ...
        # nopt_error1 = q_sum_nopt + joint_q_evals.detach() - q_total_targets.detach()
        # nopt_error2 = q_sum_nopt.detach() + joint_q_evals - q_total_targets.detach()
        # nopt_error1 = nopt_error1.clamp(max=0)
        # nopt_error2 = nopt_error2.clamp(min=0)
        # l_nopt1 = ((nopt_error1 * mask) ** 2).sum() / mask.sum()
        # l_nopt2 = ((nopt_error2 * mask) ** 2).sum() / mask.sum()
        ...
        if train_step > 0 and train_step % self.args.target_update_cycle == 0:
            self.target_rnn.load_state_dict(self.eval_rnn.state_dict())
            self.target_joint_q.load_state_dict(self.eval_joint_q.state_dict())
            self.qmix_net.load_state_dict(self.target_qmix_net.state_dict())
    ...
    def get_qtran(self, batch, hidden_evals, hidden_targets, local_opt_actions, hat=False):
        ...
        if hat:
            ...
            q_evals = self.eval_joint_q(states, local_opt_actions)
            q_targets = None
            v = None
            ...
            q_evals = self.eval_joint_q(states, u_onehot)
            q_targets = self.target_joint_q(states_next, local_opt_actions)
            v = self.v(states, hidden_evals)
            # 把q_eval、q_target、v维度变回(episode_num, max_episode_len)
            q_evals = q_evals.view(episode_num, -1, 1).unsqueeze(-1)
            q_targets = q_targets.view(episode_num, -1, 1).unsqueeze(-1)
    ...
    def save_model(self, train_step):
        ...
        torch.save(self.qmix_net.state_dict(), self.model_dir + '/' + num + '_qmix_params.pkl')
