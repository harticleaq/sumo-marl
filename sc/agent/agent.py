...
class Agents:
    def __init__(self, args):
        ...
        if args.alg == 'vdn':
            ...
            from policy.qlmix import QLMIX
            self.policy = QLMIX(args)
        ...
        elif args.alg == 'quantilemix':
            from policy.quantile_mix import QuantileMix
            self.policy = QuantileMix(args)
        elif args.alg == 'qjt_haq':
            from policy.qjt_haq import Qjt_haq
            self.policy = Qjt_haq(args)
        elif args.alg == 'q_MAVEN':
            from policy.q_maven import MAVEN
            self.policy = MAVEN(args)
        elif args.alg == 'qtran_haq_base':
            from policy.qtran_haq_base import QtranBase
            self.policy = QtranBase(args)
        elif args.alg == 'qtran_haq_base2':
            from policy.qtran_haq_base2 import QtranBase
            self.policy = QtranBase(args)
        elif args.alg == 'qtran_haq':
            from policy.qtran_haq import QtranAlt
    ...
    def choose_action(self, obs, last_action, agent_num, avail_actions, epsilon, maven_z=None, evaluate=False):
        ...
        # get q value
        if self.args.alg == 'maven' or self.args.alg == 'q_MAVEN':
    ...
    def train_q(self, batch, train_step, epsilon=None):
        max_episode_len = self._get_max_episode_len(batch)
        for key in batch.keys():
            if key != 'z':
                batch[key] = batch[key][:, :max_episode_len] 
        self.policy.learn_q(batch, max_episode_len, train_step, epsilon)
