...
class Qatten_Weight(nn.Module):
    ...
    def forward(self, agent_qs, states, actions):
        ...
        if self.args.weighted_head:
            ...
            w_head = w_head.view(-1, self.n_head, 1).repeat(1, 1,
                                                            self.n_agents)  # w_head: (bs, head_num, self.n_agents)
            head_attend *= w_head
        ...
        head_entropies = [(-((probs + 1e-8).log() * probs).squeeze().sum(1).mean()) for probs in
                          head_attend_weights]
